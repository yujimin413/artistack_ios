...
class VideoCacheManager: NSObject {
    ...
    private func clearDiskCache() -> String{
        do {
            ...
            // Unit : MB
            let folderSizeMb = ByteCountFormatter()
            folderSizeMb.allowedUnits = [.useMB]
            folderSizeMb.countStyle = .file
            let string = folderSizeMb.string(fromByteCount: Int64(folderSize))
            return string
            //String.format(decimal: folderSize/1024.0/1024.0) ?? "0"
        } catch {
    ...
    // MARK: - Secure Hashing
    // Got Disk Cache Path : encrypting the key with SHA-2 in pathName
